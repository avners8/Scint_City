...
%   psf         - psf of the structure (a 1D vector)
    ...
    N_3rdD = 1001;
    theta = linspace(-pi/2, pi/2, N_3rdD);
    ...
    % calculating x
    [z, max_Nz] = compute_b(d,i_scint,N_3rdD,control.is_Gz,control.dz_Nz);
    x    = zeros(size(f));
    mask = zeros(size(x));
    for i = 1 : length(i_scint)
       for b_i = 1 : max_Nz
           b = z(i, b_i,1);
           theta1 = asin((n(i) ./ n(end)) .^2 .* sin(theta));
           mask(i, b_i, :) = (theta1 == real(theta1));           
           x(i, b_i, :)    = Ts(d,n,i_scint(i),i_scint(i),b,h,theta1 .* (squeeze(mask(i, b_i, :))).');
       end
   end
    
   right_limit = max(x(:,:,1:floor(0.9*N_3rdD)),[],'all');
...
function x = Ts(d,n,i_scint,layer_index,z,h,theta)
    if layer_index == i_scint
        ...
        theta2 = asin((n(layer_index-1) ./ n(layer_index)) .^2 .* sin(theta));
